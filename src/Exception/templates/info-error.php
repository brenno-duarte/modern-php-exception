<?php if ($this->type == 'error') : ?>
    <?php foreach ($this->info_exception as $info) : ?>
        <section class="hidden bg-main-info-exc rounded-3" id="<?= strtolower(pathinfo($info['file'])['filename'] . str_replace(['#', '{', '}', '(', ')'], '', $info['function'])) ?>">
            ...
            <pre><code class="language-php code-exception p-3 <?= strtolower(pathinfo($info['file'])['filename'] . str_replace(['#', '{', '}', '(', ')'], '', $info['function'])) ?>"><?= htmlentities(file_get_contents($info['file'])) ?></code></pre>

            <div class="accordion accordion-flush" id="accordionError">
                <div class="accordion-item">
                    <h2 class="accordion-header" id="<?= strtolower(pathinfo($info['file'])['filename'] . str_replace(['#', '{', '}', '(', ')'], '', $info['function'])) ?>">
                    ...
                    <div id="collapseError" class="accordion-collapse collapse show" aria-labelledby="errorDetails" data-bs-parent="#accordionError">
                        <div class="accordion-body">
                            ...
                            <?php if (is_string($info['code']) && !empty($info['code'])) : ?>
